...
Проверить базовые поведенческие тесты:
...
- **Расписание на учебный год** (`/schedule`) — 2026/2027 год, ~166 учебных дней,
  темы распределены по дням недели отдельно для каждого класса
  (`content/schedule-5.json` и `content/schedule-6.json`, генерируются
  `pipeline/build_schedule.py`).
...
- **Пользователи и логин** — если задана переменная окружения
  `EDU_PORTAL_PASSWORD`, сайт закрыт логин-страницей (`/login`), а прогресс
  каждого пользователя хранится отдельно. При первом запуске создаётся аккаунт
  из `EDU_PORTAL_LOGIN` (по умолчанию `admin`) и `EDU_PORTAL_PASSWORD`; пароль
  хранится только как PBKDF2-хеш. Дополнительных пользователей создаёт команда
  `python pipeline/add_user.py <login>` (пароль запрашивается интерактивно).
  Без `EDU_PORTAL_PASSWORD` авторизация отключена для локальной разработки,
  используется пользователь с ID 1.
...
Чтобы добавить второго ребёнка после первого запуска:

```bash
cd /var/www/edu-portal
.venv/bin/python pipeline/add_user.py child
```
